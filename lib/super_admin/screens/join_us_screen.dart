...
class _JoinUsScreenState extends State<JoinUsScreen> {
  ...
  List<Map<String, dynamic>> floorSlotDto = [];
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              Expanded(
                  child: _buildTextFieldWithIcon(
                      Icons.person, 'Select your country', countryName)),
              SizedBox(width: 16),
              Expanded(
                  child: _buildTextFieldWithIcon(
                      Icons.person, 'Select your state', stateName)),
            ],
          ),
          SizedBox(height: 16),
          Row(
            children: [
              Expanded(
                  child: _buildTextFieldWithIcon(
                      Icons.person, 'Select your District', districtName)),
              SizedBox(width: 16),
              Expanded(
                  child: _buildTextFieldWithIcon(
                      Icons.home, 'Property name', propertyName)),
            ],
          ),
          SizedBox(height: 16),
          _buildTextFieldWithIcon(Icons.person, 'Admin name', adminName),
          SizedBox(height: 16),
          _buildTextFieldWithIcon(Icons.person, 'City name', cityName),
          SizedBox(height: 16),
          ...
          _buildTextFieldWithIcon(
              Icons.flood_rounded, 'Number of floors', floors),
          SizedBox(height: 24),
        ...
        child: Wrap(
          ...
          children: selectedRoles.map((role) {
            return Chip(
              ...
              onDeleted: () {
                setState(() {
                  selectedRoles.removeWhere((selectedRole) => selectedRole['roleName'] == role['roleName']);
                });
              },
            );
          }).toList(),
              ...
              content: SingleChildScrollView(
                child: ListBody(
                  children: roles.map((role) {
                    bool isSelected = selectedRoles.any((selectedRole) => selectedRole['roleName'] == role['roleName']);
              ...
              actions: [
                TextButton(
                  ...
                  onPressed: () {
                    ...
                    //print("Selected roles and responsibilities: $selectedRoles");
                  },
                ),
              ],
            );
    ...
    return ElevatedButton(
      onPressed: () {
        if (adminName.text.isEmpty ||
            ...
            districtName.text.isEmpty ||
            propertyName.text.isEmpty ||
            email.text.isEmpty ||
            payment.text.isEmpty ||
            hour.text.isEmpty ||
            floors.text.isEmpty ||
            slots.text.isEmpty ||
          ...
          String floorNames = floors.text.toString();
          String slotsNames = slots.text.toString();

          List<String> floorsLi = floorNames.split(', ');

          List<List<String>> slotsLi = slotsNames
              .split(RegExp(r'\),\s*\('))
              .map((e) => e.replaceAll(RegExp(r'[()]'), '').split(','))
              .toList();

          for (int i = 0; i < floorsLi.length; i++) {
            if (i < slotsLi.length) {
              floorSlotDto.add({
                'slotNumber': slotsLi[i], // List of slot numbers
                'floor': floorsLi[i], // Corresponding floor name
              });
            }
          }
          ...
          ratesData.add({
            'googleLocation' : location.text.toString(),
            'duration' : hour.text.toString(),
            'charge' : payment.text.toString()
          });

          for (var role in selectedRoles) {
            role["adminName"] = adminName.text.toString();
          }

          submitData();
          /*customerData.add({
            'floorSlotDto': floorSlotDto,
            'propertyName' : propertyName.text.toString(),
            'city' : cityName.text.toString(),
            'district' : districtName.text.toString(),
            'state' : stateName.text.toString(),
            'country' : countryName.text.toString(),
            'slotAvailability' : 'true',
            'googleLocation' : location.text.toString(),
            'adminName' : adminName.text.toString(),
            'userDto' : userData,
            'roleDto' : selectedRoles,
            'ratesDto' : ratesData
          });*/
          //print(customerData);
        }
